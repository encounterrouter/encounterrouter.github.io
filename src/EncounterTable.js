        ...
        <>
            <div style={{ position: 'fixed', width: '8vw', left: "30vw", top: "10vh", display: "flex", flexDirection: "column" }}>
                ...
                {SplitData.splits.map(split =>
                    <button style={{ marginTop: '1vh' }} onClick={() => document.querySelector("#" + split.name).scrollIntoView()}>{split.name}</button>
                )}
            </div>
            <div key={"Table"} className="encounterTable">
                {SplitData
                    ?
                    SplitData.splits.map(split => <Fragment key={split.name}>
                        <div id={split.name} style={{ fontSize: 'calc(10px + 8vh)', paddingTop: '10vh', paddingBottom: '3vh' }}>{split.name} Split</div>
                        {split.locations.map(location => <Fragment key={location}>
                    ...
                    :
                    locations.map(location => <Fragment key={location.name}>
